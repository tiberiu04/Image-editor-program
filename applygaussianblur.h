...
#include "structs.h"
#ifndef APPLYGAUSSIANBLUR_H
#define APPLYGAUSSIANBLUR_H

void applygaussianblur(double ****temp, selection *select, image *img)
{
	double kernel[3][3];
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			kernel[i][j] = 2; // kernelul standard
	kernel[0][0] = 1, kernel[2][0] = 1, kernel[0][2] = 1, kernel[2][2] = 1;
	kernel[1][1] = 4;
	...
	for (int i = select->y1; i < select->y2; i++)
		for (int j = select->x1; j < select->x2; j++) {
			// verific daca are 8 vecini ca sa pot aplica filtrul
			if (i > 0 && i < m - 1 && j > 0 && j < n - 1) {
