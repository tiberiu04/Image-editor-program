...
#include <math.h>
#include "structs.h"
#ifndef APPLYSHARPEN_H
#define APPLYSHARPEN_H

void applysharpen(double ****temp, selection *select, image *img)
{
	double kernel[3][3];
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			kernel[i][j] = -1;
	kernel[0][0] = 0, kernel[2][0] = 0, kernel[0][2] = 0, kernel[2][2] = 0;
	kernel[1][1] = 5; //initializez kernelul standard
	double red, green, blue;
	int m = img->height;
	int n = img->width;
	for (int i = select->y1; i < select->y2; i++) //aplicam filtrul pe selectie
